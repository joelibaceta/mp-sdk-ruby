describe MercadoPago do
  context "Strong Variable" do
    let (:dummy_variable) { StrongVariable.new({name: "dummy",
                                                type: String,
                                                length: 20,
                                                read_only: false
                                               })
    ...
    let (:read_only_variable) { StrongVariable.new({name: "read_only",
                                                    type: String,
                                                    read_only: true
                                                  })
    }
    ...
    it "should return a hash with the parameters" do
      expect(dummy_variable.to_hash).to eql({name: "dummy",
                                             type: String,
                                             length: 20,
                                             read_only: false
                                            })
